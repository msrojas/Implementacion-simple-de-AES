/*
    Copyright (c) 2015 Alejandro Torres Hernandez

    This file is part of Implementacion simple de AES.

    Implementacion simple de AES is free software: you can redistribute it and/or modify it under the terms of the GNU 
    General Public License as published by the Free Software Foundation, either version 3 of the 
    License, or (at your option) any later version.

    Implementacion simple de AES is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without 
    even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR 
    PURPOSE. See the GNU General Public License for more details.

    You should have received a copy of the GNU General Public License along with Foobar. If not, 
    see http://www.gnu.org/licenses/.

*/
#include "desencripta.h"

uint8_t inverse_Sbox[256] = 
{

};

uint8_t mix_column_14[256] = 
{

};

uint8_t mix_column_13[256] =
{

};

uint8_t mix_column_11[256] =
{

};

uint8_t mix_column_9[256] = 
{

};

uint8_t inverse_mix_column[16] = {
14, 11, 13, 9,
9, 14, 11, 13,
13, 9, 14, 11,
11, 13, 9, 14
};

void print_block_as_int(uint8_t buffer[])
{
    uint8_t i;
    for(i=0;i<16;i++)
        printf("%d ", buffer[i]);
    printf("\n");
}

inline get_inverse_Sbox(uint8_t num)
{
    return inverse_Sbox[num];
}

void inverse_MixColumn(uint8_t * block)
{
    short bits = 0;
    uint8_t j = 0, i, ii;
    uint8_t temp[4], block_de_4bits[4];
    uint8_t indice = 0, byte = 0;
    uint8_t mask = 0;

    memset(temp, 0, sizeof(temp));
    memset(block_de_4bits, 0, sizeof(block_de_4bits));

    for(i=0;i<16;i++)
    {
        temp[indice++] = block[bits+j];

        bits = bits + 4;

        if((i == 3) || (i == 7) || (i == 11) || (i == 15))
        {
            indice = 0;
            for(ii=0;ii<16;ii++)
            {
                byte = temp[indice];
                if(inverse_mix_column[ii] == 14)
                    byte = mix_column_14[byte];
                else if(inverse_mix_column[ii] == 13)
                    byte = mix_column_13[byte];
                else if(inverse_mix_column[ii] == 11)
                    byte = mix_column_11[byte];
                else if(inverse_mix_column[ii] == 9)
                    byte = mix_column_9[byte];
		
                block_de_4bits[indice++] = byte;

                if(indice == 4)
                {
                    mask = ((block_de_4bits[0] ^ block_de_4bits[1]) ^ block_de_4bits[2]) ^ block_de_4bits[3];

                    if(ii == 3)
                        block[0+j] = mask;
                    else if(ii == 7)
                        block[4+j] = mask;
                    else if(ii == 11) 
                        block[8+j] = mask;
                    else if(ii == 15)
                        block[12+j] = mask;

                    memset(block_de_4bits, 0, sizeof(block_de_4bits));
                    indice = 0;
                }
            }
            j++;
            bits = 0;
            memset(temp, 0, sizeof(temp));
       }

    }

}

void inverse_byte_substitution(uint8_t * block)
{
    uint8_t i;
    uint8_t byte = 0;
    for(i=0;i<16;i++)
    {
        byte = get_inverse_Sbox(block[i]);
        block[i] = byte;
    }
}

void inverse_shift_row(uint8_t * block)
{
    uint8_t i;
    uint8_t temp = block[7];
    for(i=7;i>4;i--) //SEGUNDA LINEA. 1 BYTE RIGHT SHIFT
        block[i] = block[i-1];
    block[4] = temp;

    //TERCERA LINEA. 2 BYTE RIGHT SHIFT
    temp = block[11];
    block[11] = block[9];
    block[9] = temp;
    temp = block[10];
    block[10] = block[8]; 
    block[8] = temp;

    //CUARTA LINEA. 3 BYTE RIGHT SHIFT
    temp = block[12];
    for(i=12;i<15;i++)
        block[i] = block[i+1];
    block[15] = temp;
}

uint8_t * rounds_des(uint8_t block[], unsigned char * expanded_key)
{
    short bits_key = 160, limit_key = 176;
    uint8_t i, ii,indice = 0;
    uint8_t * cipher = NULL;

    for(i=bits_key;i<limit_key;i++) //Invertimos add round key
        block[indice++] ^= expanded_key[i];

    inverse_shift_row(block);
    inverse_byte_substitution(block);
	
    indice = 0;
    bits_key -= 16;
    limit_key -= 16;

    for(i=0;i<9;i++)
    {
        for(ii=bits_key;ii<limit_key;ii++) //Invertimos add round key
            block[indice++] ^= expanded_key[ii];

        inverse_MixColumn(block);
        inverse_shift_row(block);
        inverse_byte_substitution(block);

        bits_key -= 16;
        limit_key -= 16;
        indice = 0;
    }	

    for(i=bits_key;i<limit_key;i++) //Invertimos add round key
        block[indice++] ^= expanded_key[i];


    cipher = (char *)malloc(16+1);
    if(cipher == NULL)
    {
        fprintf(stderr, "Error: ocurrio un inconveniente en la heap\n");
        exit(1);
    }

    for(ii=0;ii<16;ii++)
        cipher[ii] = block[ii];
    cipher[17] = '\0';

    return cipher;
}

uint8_t * desencripta(unsigned char * cipher, unsigned char * expanded_key, int len)
{
    int i;
    uint8_t block_de_128bits[16];
    uint8_t indice = 0;
    uint8_t * cipher_block = NULL;
    uint8_t * return_cipher = (char *)malloc(len+1);
    uint8_t ii, t = 0;

    memset(block_de_128bits, 0, sizeof(block_de_128bits));

    for(i=0;i<len;i++)
    {
        block_de_128bits[indice++] = cipher[i];

        if(indice == 16)
        {
            cipher_block = rounds_des(block_de_128bits, expanded_key);

            for(ii=0;ii<16;ii++)
                return_cipher[t++] = cipher_block[ii];

            indice = 0;
            memset(block_de_128bits, 0, sizeof(block_de_128bits));
            free(cipher_block);
        } 
    }

    return_cipher[t] = '\0';
    return return_cipher;
}